 ...
 * State is base64-encoded and may contain: "redirectUri" or "redirectUri|returnPath"
 */
function parseState(state: string): { redirectUri: string; returnPath: string } {
  try {
    ...
    const redirectUri = decoded.substring(0, pipeIndex);
    const returnPath = decoded.substring(pipeIndex + 1) || "/";
...
export function registerOAuthRoutes(app: Express) {
  app.get("/api/oauth/callback", async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Parse state to extract return path (e.g., /admin)
      ...
      console.log(`[OAuth] Login successful for ${userInfo.openId}, redirecting to: ${safePath}`);
